...
<div id="webcam_archive">
	...
	<?php else : ?>
		<h2><?php echo date('F j, Y', $first_image_date + $blog_offset); ?><form action="http://<?php echo $_SERVER['SERVER_NAME']; ?><?php echo $_SERVER['SCRIPT_NAME']; ?>" method="get"><input type="text" id="datepicker" class="datepicker" name="date" value="<?php echo date('Y-m-d', key($entry_array) + $blog_offset); ?>" />
						...
						if ($key == 0) {
							?>
								<div class="thumb">
									...
									<?php echo date('g:i a', $entry_date + $blog_offset); ?>
								</div>
							<?php
						} elseif ($key == 1) {
							?>
